...
_EXPANSIONS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\bwithout\s+notif", re.I), "notify notification revised terms posting email"),
    (re.compile(r"\bchange(s)?\b.*\bterms\b|\bterms\b.*\bchang", re.I), "revised notify notification posting reasonable means"),
    (re.compile(r"\bwho owns\b|\bownership\b", re.I), "license grant retain intellectual property content"),
    (re.compile(r"\bgoverning\s+law\b|\bjurisdiction\b", re.I), "california delaware court venue limitation legal action claims"),
